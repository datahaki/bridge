...
/* package */ class RangeSliderUI extends BasicSliderUI {
  ...
  // ---
  // false for lowerDragging, true for upperDragging
  private Boolean lastDragging = null;
  ...
  public void paint(Graphics graphics, JComponent c) {
    ...
    if (upperThumbSelected) {
      ...
      if (clipRect.intersects(thumbRect)) {
        PaintThumb.using(graphics, thumbRect);
      }
      if (clipRect.intersects(upperThumbRect)) {
        PaintThumb.using(graphics, upperThumbRect);
      }
    } else {
  ...
  /** Paints the track. */
  @Override
  public void paintTrack(Graphics _g) {
    // Draw track
    super.paintTrack(_g);
    ...
    if (slider.getOrientation() == SwingConstants.HORIZONTAL) {
      ...
      graphics.fillRect(lowerX, trackRect.y, upperX - lowerX, trackRect.height);
    } else {
      // Determine position of selected range by moving from the middle
      // of one thumb to the other.
      ...
      // Draw selected range.
      graphics.setColor(rangeColor);
      graphics.fillRect(trackRect.x, upperY, trackRect.width, lowerY - upperY);
    }
    graphics.dispose();
  }
